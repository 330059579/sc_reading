...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	...
	//todo documentReaderClass 的默认值为 DefaultBeanDefinitionDocumentReader.class
	 ...
	 * if none specified.
	 */
	// todo 返回指定的解析器，如果没有指定，则构造一个未指定的默认解析器。
	// todo EntityResolver 有四个实现类 BeansDtdResolver ，PluggableSchemaResolver ，DelegatingEntityResolver   ，ResourceEntityResolver， 这里默认使用DelegatingEntityResolver
	// todo EntityResolver 的作用就是，通过实现它，应用可以自定义如何寻找【验证文件】的逻辑。
	...
	public int loadBeanDefinitions(Resource resource) throws BeanDefinitionStoreException {
		//todo Resource 资源封装成 org.springframework.core.io.support.EncodedResource 对象
	...
	public int loadBeanDefinitions(EncodedResource encodedResource) throws BeanDefinitionStoreException {
		...
		//todo 将当前资源加入记录中。如果已存在，抛出异常, currentResources是set类型，
		// 如果encodedResource中的值完全相同，则不能添加
		...
		try {
			//todo  从 EncodedResource 获取封装的 Resource ，并从 Resource 中获取其中的 InputStream
			...
			try {
				...
				//todo  核心逻辑部分，执行加载 BeanDefinition
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
			throws BeanDefinitionStoreException {
		try {
			//todo 分析获取 xml 文件的验证模式 这样才能保证xml文件的正确性
			int validationMode = getValidationModeForResource(resource);
			//todo 根据 xml 文件，获取 Document 实例。
			Document doc = this.documentLoader.loadDocument(
					inputSource, getEntityResolver(), this.errorHandler, validationMode, isNamespaceAware());
			//todo 获取到解析完成Document对象之后，开始注册 BeanDefinitions
	...
	protected int getValidationModeForResource(Resource resource) {
		...
		// todo 最后，使用 VALIDATION_XSD 做为默认
	...
	protected int detectValidationMode(Resource resource) {
		//todo 不可读，抛出 BeanDefinitionStoreException 异常
		if (resource.isOpen()) {
			throw new BeanDefinitionStoreException(
	...
	public int registerBeanDefinitions(Document doc, Resource resource) throws BeanDefinitionStoreException {
		//todo <1> 创建 BeanDefinitionDocumentReader 对象 调用createBeanDefinitionDocumentReader() 方法，实例化 BeanDefinitionDocumentReader 对象。
		BeanDefinitionDocumentReader documentReader = createBeanDefinitionDocumentReader();
		documentReader.setEnvironment(getEnvironment());
		//todo <2> 获取已注册的 BeanDefinition 数量  getRegistry方法返回BeanDefinitionRegistry对象，其中getBeanDefinitionCount方法返回了map的数量
		int countBefore = getRegistry().getBeanDefinitionCount();
		//todo <3> 创建 XmlReaderContext 对象
		//todo  <4> 注册 BeanDefinition      createReaderContext(Resource resource) 方法，创建 XmlReaderContext 对象
		documentReader.registerBeanDefinitions(doc, createReaderContext(resource));
		//todo 计算新注册的 BeanDefinition 数量
	...
	protected BeanDefinitionDocumentReader createBeanDefinitionDocumentReader() {
		//todo documentReaderClass 的默认值为 DefaultBeanDefinitionDocumentReader.class
		//todo cast 方法返回他的父类 BeanDefinitionDocumentReader
	...
	protected XmlReaderContext createReaderContext(Resource resource) {
	    //todo a1 NamespaceHandlerResolver在此初始化
		if (this.namespaceHandlerResolver == null) {
			this.namespaceHandlerResolver = createDefaultNamespaceHandlerResolver();
		}
		//todo 传到XmlReaderContext中
